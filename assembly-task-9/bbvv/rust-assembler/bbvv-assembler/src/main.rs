/***
 * Assembler for Bacon Borde Vara Vegetarisk
...
/// Run assembler application.
/// #### Returns
/// An `Result` structure where:
/// - `Ok(())` means no errors
/// - `Err(<i32>)` means exit caused by error 
fn run_assembler() -> Result<(), ErrorKind> {
    println!("--- Bacon Borde Vara Vegetarisk assembler ---\n");
                        ...
                        if line_has_code(&lines[curr_i]) {
                            ...
                            match lines[curr_i].find(" ") {
                                Some(_split_i) => {
                                    ...
                                    match assembler::run(&expr[1..(expr.len() - 1)]) {
                                        Ok(_instr) => {
                ...
                while curr_i < lines.len() {
                    if line_has_code(&lines[curr_i]) {
                        match definitions.get(lines[curr_i].split_whitespace().next().unwrap()) {
                            ...
                            None => {
                                match assembler::run(lines[curr_i].as_str()) {
...
fn main() {
    ...
    std::process::exit(match run_assembler() {
        Ok(_) => 0,
        Err(_err) => {
